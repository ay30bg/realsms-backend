...
exports.initUSDTPayment = async (req, res) => {
  const { amount } = req.body;
  const user = req.user; // assumed you have auth middleware
  ...
  try {
    const response = await axios.post(
      ...
      {
        price_amount: Number(amount),
        price_currency: "usd",         // pricing in USD
        pay_currency: "usdttrc20",     // user pays USDT TRC20
        order_id: `${user._id}-${Date.now()}`,
        order_description: `Funding wallet for ${user.email}`,
        success_url: `${FRONTEND_URL}/fund-success`,
        cancel_url: `${FRONTEND_URL}/fund-cancel`,
...
exports.usdtWebhook = async (req, res) => {
  ...
  try {
    ...
    if (payload.payment_status === "finished") {
      ...
      // Prevent double-credit (optional: track invoice IDs if needed)
      if (!payload.already_paid) {
        const user = await User.findByIdAndUpdate(
          userId,
          { $inc: { walletBalance: Number(payload.pay_amount) } }, // credit wallet
          { new: true }
        );

        console.log(
          `Wallet credited: ${payload.pay_amount} USDT → User ${userId}. New balance: ${user.walletBalance}`
        );
      }
    }
