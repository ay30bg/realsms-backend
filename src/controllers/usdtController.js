...
//   if (Number(amount) < 6) {
//     return res.status(400).json({ message: "Minimum funding amount is 6 USDT" });
//   }
...
exports.initUSDTPayment = async (req, res) => {
  ...
  // 🔒 SAFE MINIMUM (NowPayments-friendly)
  if (numericAmount < 7) {
    return res
      .status(400)
      .json({ message: "Minimum funding amount is 7 USDT" });
  }
...
exports.usdtWebhook = async (req, res) => {
  ...
  try {
    if (payload.payment_status === "finished") {
      console.log("USDT payment finished:", payload.order_id);
      ...
      console.log(`Wallet credited: ${payload.price_amount} USDT → ${userId}`);
    }

    return res.status(200).json({ ok: true });
  } catch (err) {
    console.error("USDT webhook error:", err.message);
